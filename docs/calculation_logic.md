...
目标倒推用于回答“为了让核心/综合偿付能力充足率变化若干百分点，单一资产类型需要正向变化多少”。处理逻辑：

1. 用户选择目标指标、目标变化百分点、资产类型和债券久期 bucket。
2. 程序分别求解 `加仓/建仓` 和 `上涨/下跌` 两种正向动作。
3. 搜索范围为 0 到该资产类型当前认可价值的 5 倍；先分段扫描找跨越区间，再二分求最小满足目标的金额。
4. 如果正向动作在搜索上限内无法达到目标，显示无解，不自动给出卖出或下跌的反向解。
